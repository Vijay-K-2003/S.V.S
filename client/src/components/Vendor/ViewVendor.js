...
const ViewVendor = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/vendors/${id}`)
      .then((res) => {
        setVendor(res.data);
      });
  }, [id]);
      ...
      {userObject.emails[0].value === vendor.email ? (
        <div className="container-flex-data">
          ...
          <div className="rightvv">
            <button className="btn-st" onClick={handleTracking}>
              Start Tracking
            </button>
            <Link className="btn-mc" to={`/vendors/${id}/myCustomers`}>
              <button className="btn-mc"> My Customers</button>
            </Link>
